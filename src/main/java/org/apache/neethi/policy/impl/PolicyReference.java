...
public class PolicyReference implements PolicyComponent {
    ...
    public boolean isEquivalent(PolicyComponent policyComponent) {
        if (Constants.TYPE_POLICY_REF != policyComponent.getPolicyType()) {
            return false;
        }
    ...
    public PolicyComponent normalize(PolicyRegistry reg, boolean deep) {
        ...
        if (policy == null) {
            ...
            if (policy == null) {
                throw new PolicyException(key + " can't be resolved");
            }
    ...
    public Policy getRemoteReferencedPolicy(String u) {
        try {
            ...
            throw new PolicyException("Malformed uri: " + u);
        } catch (IOException ioe) {        
            throw new PolicyException("Cannot reach remote resource: " + u);
        }
    }
}
