...
import org.apache.neethi.policy.IntersectableAssertion;
...
public class PolicyIntersector {
    ...
    All createCompatibleAlternatives(Collection<? extends PolicyComponent> alt1, 
		...
		if(!searchAlternative(alt1, alt2, remove, all) || !searchAlternative(alt2, alt1, false, all)) {
			return null;
		}
        return all;
    }
	private boolean searchAlternative(Collection<? extends PolicyComponent> policy1,
			Collection<? extends PolicyComponent> policy2, boolean remove, All all) {
		Iterator<? extends PolicyComponent> iterator = policy1.iterator();
        while (iterator.hasNext()) {
            PolicyComponent a1 = iterator.next();
            if (a1 instanceof Assertion) {
                Assertion assertion = findCompatibleAssertion((Assertion)a1, policy2, remove);
                if (assertion != null) {
                    if (remove) {
                        iterator.remove();
                    }
